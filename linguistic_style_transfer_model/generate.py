...
class SqliteSet:
    def __init__(self, dbpath='nickname.db', tablename='nickname', colname='name'):
        import sqlite3
        self.conn = sqlite3.connect(dbpath)
        self.cur = self.conn.cursor()
        # self.cur.execute("PRAGMA cache_size=80000;")
        # self._create_table(self.conn)
        self.tablename = tablename
        self.colname = colname
        self._cache = []

        self.contain_cmd = f"select {self.colname} from {self.tablename} where {self.colname}=?"
        self.insert_cmd = f"insert or ignore into {self.tablename} ({self.colname}) values(?)"

        self.n = self.cur.execute(f'select count(*) from {self.tablename}').fetchall()[0][0]
    
    # def _create_table(self, conn):
    #     cur = conn.cursor()
    #     existing_tables = [x[0] for x in cur.execute('select name from sqlite_master where type="table";').fetchall()]
    #     if 'nickname' in existing_tables:
    #         return
    #     cur.execute("""create table nickname (name TEXT NOT NULL PRIMARY KEY);""")
    #     conn.commit()
    
    def add(self, key, batch=True):
        # if key in self:
        #     return False
        lastrowid = self.cur.lastrowid or \
            self.cur.execute(f'select MAX(_ROWID_) from {self.tablename}').fetchall()[0][0]

        if not batch:
            self.cur.execute(self.insert_cmd, key)
            self.conn.commit()
        else:
            self._cache.append( (key, ) )
            if len(self._cache) == 1000:
                self.cur.execute("BEGIN")
                self.cur.executemany(self.insert_cmd, self._cache)
                self.conn.commit()
                self._cache.clear()
        
        cur_lastrowid = self.cur.lastrowid or \
            self.cur.execute(f'select MAX(_ROWID_) from {self.tablename}').fetchall()[0][0]
        accepted = cur_lastrowid - lastrowid
        self.n += accepted

        return accepted

    def __contains__(self, key):
        if len(self.cur.execute(self.contain_cmd, (key, )).fetchall()):
            return True
        return False
...
def main(argv):
    ...
    if options.generate_novel_text:
        ...
        parser.add_argument("--label-index", type=int, default=1, required=False)
        parser.add_argument("--sqlite3_path", type=str, default='nickname.db', required=False)
        parser.add_argument("--sqlite3_tablename", type=str, default='nickname', required=False)
    ...
    if options.train_model:
        ...
        saver.restore(sess=sess, save_path=os.path.join(options.saved_model_path, global_config.model_save_file))
        
        generated = SqliteSet(options.sqlite3_path, options.sqlite3_tablename)
        n_generated = len(generated)
        n_accepted = 0
        n_newadd = 0

        label_index = options.label_index

        average_label_embedding = average_label_embeddings[int(label_index)]
        adv = average_label_embeddings[0]

        average_label_embedding = average_label_embedding * 0.3 + adv * 0.7

        while n_generated < options.num_sentences_to_generate:
            
            style_embedding = np.asarray(average_label_embedding)
            style_embedding += np.random.normal(style_embedding.shape) * 0.01

            generated_sequences, final_sequence_lengths = \
                network.generate_novel_sentences_initialized(
                    sess, style_embedding, 2048, num_labels,
                    )
            ...
            for sentence in generated_sentences:
                sentence = sentence.replace(' ', '').replace('unk', '')
                n_newadd += 1

                n_acc = generated.add(sentence)
                ...
                if n_acc:
                    ...
                    if True:
                        logger.info("Generated {} sentences. Recent: {} ({} / {})".format(
                            n_generated, sentence, n_accepted, n_newadd
                        ))

                        n_accepted = n_newadd = 0
